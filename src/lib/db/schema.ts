...
export const pokemonToTypeRelations = relations(pokemonToType, ({ one }) => ({
  pokemon: one(pokemon, { fields: [pokemonToType.pokemonId], references: [pokemon.id] }),
  type: one(pokemonType, { fields: [pokemonToType.typeId], references: [pokemonType.id] }),
}));
...
export const card = pgTable('card', {
  ...
  cardTypeId: uuid('card_type_id').references(() => cardType.id).notNull(),
  createdAt: timestamp('created_at').defaultNow(),
  updatedAt: timestamp('updated_at').$onUpdate(() => new Date()),
});
...
export const cardRelations = relations(card, ({ one }) => ({
  ...
  cardType: one(cardType, { fields: [card.cardTypeId], references: [cardType.id] }),
}));
